...
export function ArticleEditor({ article, mode }: ArticleEditorProps) {
		...
		startTransition(async () => {
			try {
				...
				if (mode === "create") {
					...
					await updateArticleMutation({ data: { id: article.id, input } });
					setHasUnsavedChanges(false);
					router.invalidate();
				}
	...
	const handleTogglePublish = () => {
		...
		startTransition(async () => {
			try {
				if (published) {
					await unpublishArticleMutation({ data: { id: article.id } });
					setPublished(false);
				} else {
					await publishArticleMutation({ data: { id: article.id } });
					setPublished(true);
				}
				router.invalidate();
	...
	const handleDelete = () => {
		if (!article?.sha || !window.confirm("Delete this article?")) return;
		startTransition(async () => {
			try {
				await deleteArticleMutation({
					data: { id: article.id, sha: article.sha },
				});
				router.invalidate();
